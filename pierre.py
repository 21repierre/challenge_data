...
class Model1(torch.nn.Module):
    ...
    def __init__(self, hidden=20, lstm_layers=2, nns=[(20, 0.5)]):
        ...
        self.conv = torch.nn.Sequential(
            torch.nn.Conv1d(478, hidden, kernel_size=3, padding=1),
            torch.nn.BatchNorm1d(hidden),
            torch.nn.ReLU(),
            torch.nn.MaxPool1d(2),

            torch.nn.Conv1d(hidden, hidden // 2, kernel_size=3, padding=1),
            torch.nn.BatchNorm1d(hidden // 2),
            torch.nn.ReLU(),
            # torch.nn.MaxPool1d(2),
        )
        self.linear_after = torch.nn.Sequential(torch.nn.Linear(hidden // 2, nns[0][0]), torch.nn.ReLU())
